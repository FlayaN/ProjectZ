...
static void fontToTexture()
{
	...
	for (i = 0, j = 'A'; i < 26; i++, j++)
		charToTexture((char*)letters[i], j, data);
	// Lower case
	for (i = 0,j = 'a'; i < 26; i++,j++)
		charToTexture((char*)lowletters[i], j, data);
	// Numbers and some more
	for (i = 0,j = '0'; i < 16; i++,j++)
		charToTexture((char*)numletters[i], j, data);
	// Space and some more
	for (i = 0,j = ' '; i < 16; i++,j++)
		charToTexture((char*)specialletters[i], j, data);
...
/*void sfDrawString(int h, int v, const char *s, ...)
{
	...
	if (h < 0)
		h = gRasterH + h - strlen(buffer)*10;
	if (v < 0)
		v = gRasterV + v; // * 13;
	for(;*buffer != 0;buffer++)
	{
		drawChar(h, v-10, *buffer); // -10 to get to the proper height
